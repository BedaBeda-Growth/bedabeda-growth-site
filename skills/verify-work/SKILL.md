---
name: verify-work
description: "Compare completed work in a thread against the plan it's executing on, or if no plan is referenced, verify the work done in the current thread. Use when an agent (Jules, Antigravity, or Kai) has finished a task and needs verification before marking done or handing to human QA. Checks — (1) rule compliance across all domains the work touches, (2) full AC coverage (or thread objective coverage), (3) self-verification quality — proof is measurable (test output, query result, API response, screenshot) not prose-only. Human QA is a final polishing layer — LLMs must get the work to functional and verified first. Triggers on — verify this work, review this work, check against the plan, does this meet the AC, self-verify, QA check, verify before done, is this complete, verify before merge."
---

# Verify Work

Verify completed work against its backing plan or thread context. Produce a structured verdict. Human QA is polish — not primary validation.
...
## Step 1: Load the Plan (or Thread Context)
...
**If no plan is identifiable, DO NOT STOP.** Instead, dynamically verify the work that was actually performed and discussed within the current thread. Evaluate if the actions meet the user's explicit or implicit requests and if the code changes are sound and verified according to standard practices. Derive the "Acceptance Criteria" from the thread's goals.
...
```
## Work Verification: [plan/task name or thread goal]
